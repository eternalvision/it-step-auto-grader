...
- MutationObserver-сигналы для появления и изменения форм с timeout fallback;
...
Кнопка **Стоп** устанавливает флаг остановки. Текущий observer-driven wait или
ожидание с timeout fallback завершается сразу, после чего новая форма не берётся. Grade и
...
комментарием, перед submit и внутри ожиданий. Ожидания подписываются на
`childList`, `disabled`, `aria-disabled`, `aria-checked` и `class`; каждый
одноразовый observer отключается после успеха, остановки или таймаута, а timer
остаётся fallback для изменений, которые не породили mutation.
...
```bash
node --check index.js
node --test test/mutation-observer.test.js
```
